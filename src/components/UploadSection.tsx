  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/20 pb-5">
        ...
        <div className="flex items-center gap-2.5">
          <button
            ...
            className="bg-[#E2E4E8] text-zinc-950 font-bold px-4 py-2 rounded-full text-xs sm:text-sm hover:bg-[#FF2A3A] hover:text-white active:scale-95 transition-all duration-200 flex items-center gap-2 cursor-pointer shadow-sm"
          ...
          <button
            ...
            className="border border-white/20 bg-white/5 text-white font-semibold px-4 py-2 rounded-full text-xs sm:text-sm hover:bg-[#FF2A3A] hover:text-white hover:border-[#FF2A3A] active:scale-95 transition-all duration-200 flex items-center gap-2 cursor-pointer"
      ...
      <div
        ...
        onDrop={handleDrop}
        onClick={() => fileInputRef.current?.click()}
        className={`w-full p-8 sm:p-10 rounded-2xl border-2 border-dashed transition-all flex flex-col items-center justify-center text-center cursor-pointer relative overflow-hidden ${
          dragActive
            ? 'border-white bg-white/10 scale-[0.99]'
            : 'border-white/20 bg-white/[0.02] hover:border-white/50'
        }`}
        id="drag-drop-zone"
      >
        ...
        <h3 className="text-sm sm:text-base font-bold text-white mb-1">
          Drop your image files here, or <span className="underline underline-offset-2">browse device</span>
        </h3>
      ...
      {filteredImages.length === 0 ? (
        <div className="p-12 rounded-2xl border border-white/10 bg-white/5 text-center text-white/40">
          ...
          <button
            onClick={() => fileInputRef.current?.click()}
            className="px-4 py-2 rounded-full bg-white text-black font-bold text-xs inline-flex items-center gap-1.5"
          >
            Upload Now
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {filteredImages.map((img) => (
            <div
              ...
              className="rounded-2xl border border-white/10 hover:border-white/40 bg-white/[0.02] overflow-hidden group transition-all cursor-pointer flex flex-col justify-between shadow-md"
            >
              {/* Image Thumbnail */}
              <div className="aspect-square w-full bg-white/5 relative overflow-hidden">
                ...
                {/* Source Badge */}
                <span className={`absolute top-2.5 left-2.5 px-2 py-0.5 rounded-md text-[9px] font-bold uppercase tracking-wider backdrop-blur-md border border-white/10 ${
                  img.source === 'camera' 
                ...
                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2 p-2">
                  <button
                    ...
                    className="p-2 rounded-xl bg-white text-black hover:bg-zinc-200 transition-colors shadow"
                  ...
                  <button
                    ...
                    className="p-2 rounded-xl bg-white/20 text-white hover:bg-white hover:text-black transition-colors shadow"
                  ...
                  <button
                    ...
                    className="p-2 rounded-xl bg-red-600/80 text-white hover:bg-red-500 transition-colors shadow"
                    title="Delete Image"
                  >
        ...
        {selectedImage && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/95 backdrop-blur-md">
            ...
            >
              ...
              <div className="flex-1 overflow-auto p-4 sm:p-8 flex items-center justify-center bg-black/50 min-h-[300px]">
                <img
