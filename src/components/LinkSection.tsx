...
export const LinkSection: React.FC<LinkSectionProps> = ({
  ...
  const handleParseAndAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const fallbackItem: LinkItem = {
        id: `link-${Date.now()}`,
        url: trimmedUrl,
        title: embedId ? `${provider.toUpperCase()} Video Stream` : host,
  ...
  return (
    ...
    >
      ...
      <div className="p-5 sm:p-7 rounded-3xl bg-zinc-950/60 border border-white/20 shadow-2xl backdrop-blur-md">
        <form onSubmit={handleParseAndAdd} className="flex flex-col sm:flex-row items-stretch gap-3">
          <div className="relative flex-1">
            <LinkIcon className="w-4 h-4 text-[#FF2A3A] absolute left-4 top-1/2 -translate-y-1/2" />
            <input
              type="text"
              value={urlInput}
              ...
              required
              className="w-full bg-black/80 text-white font-medium placeholder:text-white/40 pl-11 pr-4 py-3.5 rounded-2xl outline-none border border-white/15 focus:border-[#FF2A3A] text-xs sm:text-sm transition-colors"
              id="input-paste-url"
            />
          </div>
          <button
            type="submit"
            disabled={isParsing}
            className="px-7 py-3.5 rounded-2xl bg-[#E2E4E8] text-zinc-950 font-bold text-xs sm:text-sm hover:bg-[#FF2A3A] hover:text-white active:scale-95 transition-all duration-200 flex items-center justify-center gap-2 cursor-pointer shadow-md whitespace-nowrap"
            id="btn-parse-link"
          >
            {isParsing ? (
              <>
                <div className="w-4 h-4 border-2 border-zinc-950/30 border-t-zinc-950 rounded-full animate-spin" />
                <span>Scraping Link...</span>
              </>
            ) : (
              <>
        ...
        {errorMsg && (
          <div className="mt-3 p-3 rounded-xl bg-red-500/15 border border-red-500/30 text-red-300 text-xs font-medium flex items-center gap-2">
        ...
        <div className="mt-4 pt-4 border-t border-white/10 flex flex-wrap items-center gap-2 text-xs">
          ...
          <button
            type="button"
            onClick={() => setUrlInput('https://www.youtube.com/watch?v=M7lc1UVf-VE')}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredLinks.map((item) => (
            <motion.div
              ...
              className="rounded-3xl bg-zinc-950/60 border border-white/15 hover:border-[#FF2A3A] overflow-hidden group transition-all duration-300 cursor-pointer flex flex-col sm:flex-row shadow-xl relative backdrop-blur-md"
            >
              {/* Left Thumbnail Banner styled like YouTube banner card */}
              <div className="w-full sm:w-48 md:w-52 sm:min-w-[190px] aspect-video sm:aspect-auto bg-black relative overflow-hidden flex-shrink-0">
                {item.embedThumb ? (
                  <img
                    src={item.embedThumb}
                    alt={item.title}
                  ...
                  <div className="w-full h-full min-h-[120px] flex flex-col items-center justify-center bg-zinc-900 text-white/40 gap-1.5 p-3">
                    <Film className="w-8 h-8 text-white/30" />
                    <span className="text-[10px] font-extrabold text-white/40 uppercase">{item.linkHost}</span>
                  </div>
                )}
                ...
                {item.isPlayable && (
                  <div className="absolute inset-0 bg-black/40 group-hover:bg-black/20 transition-colors flex items-center justify-center">
                    <div className="w-11 h-11 rounded-full bg-[#FF2A3A] text-white flex items-center justify-center group-hover:scale-110 transition-all duration-200 shadow-xl">
                ...
                {/* Host badge */}
                <div className="absolute bottom-2 right-2 px-2 py-0.5 rounded text-[9px] font-extrabold uppercase tracking-wider bg-black/85 text-white backdrop-blur-md border border-white/10">
                  {item.linkHost}
                </div>

                {item.isPlayable && (
                  <div className="absolute top-2 left-2 px-2 py-0.5 rounded text-[8px] font-extrabold uppercase tracking-wider bg-[#FF2A3A] text-white shadow-md">
                    Video
                  </div>
                )}
              </div>
              ...
              <div className="p-4 sm:p-5 flex-1 flex flex-col justify-between min-w-0">
                <div>
                  <h3 className="text-xs sm:text-sm font-bold text-white group-hover:text-[#FF2A3A] transition-colors line-clamp-2 mb-1.5 leading-snug">
                    {item.title}
                  </h3>
                ...
                <div className="mt-3 pt-3 border-t border-white/10 flex items-center justify-between text-xs text-white/40">
                  <span className="text-[10px] text-white/40 font-bold uppercase truncate max-w-[120px]">
                    {item.linkHost}
                  </span>
            ...
            >
              ...
              <div className="p-4 sm:p-5 border-b border-white/10 flex items-center justify-between">
                <div className="min-w-0 pr-4">
                  ...
                  <p className="text-xs text-white/40 flex items-center gap-1.5 mt-0.5">
                    <span className="text-[#FF2A3A] font-extrabold uppercase">{activePlayItem.embedProvider}</span>
                    <span>•</span>
              ...
              <div className="w-full aspect-video bg-black flex items-center justify-center">
                {activePlayItem.embedProvider === 'youtube' && activePlayItem.embedId ? (
                  <iframe
                    src={`https://www.youtube-nocookie.com/embed/${activePlayItem.embedId}?autoplay=1&rel=0&playsinline=1&controls=1`}
                  ...
                  <iframe
                    src={`https://player.vimeo.com/video/${activePlayItem.embedId}?autoplay=1&title=0&byline=0`}
                    title={activePlayItem.title}
                  ...
                  <div className="text-center p-8 text-white/40">
                    ...
                    <button
                      ...
                      className="px-5 py-2.5 rounded-full bg-[#E2E4E8] text-black font-bold text-xs hover:bg-[#FF2A3A] hover:text-white transition-all"
                    >
